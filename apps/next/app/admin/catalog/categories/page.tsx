    ...
    <PageContainer>
      ...
      <Section>
        <Panel density='dense'>
          ...
          {filtered.length === 0 ? (
            <EmptyState title='No categories found' description='Try another search query or clear active filters.' />
          ) : (
            <TableShell>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Category Name', 'Slug', 'Parent', 'Products', 'Status', 'Actions'].map((head) => (
                      <th
                        key={head}
                        scope='col'
                        style={{
