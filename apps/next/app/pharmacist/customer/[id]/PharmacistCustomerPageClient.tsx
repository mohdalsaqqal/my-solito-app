...
type PharmacistCustomerPageClientProps = {
  session: AuthSession | null
  cmsHome: CMSHome | null
}

export default function PharmacistCustomerPageClient({ session, cmsHome }: PharmacistCustomerPageClientProps) {
        ...
        {profile ? (
          <Box style={{ gap: spacing['8'] }}>
            ...
            <Text variant='caption' tone='muted'>
              {profile.customer.email} â€¢ {profile.customer.phone ?? 'No phone'}
            </Text>
            <Text variant='caption' tone='muted'>
              Tests: {profile.tests.length} â€¢ Last visit:{' '}
                ...
                profile.tests.map((test) => (
                  <Card key={test.id} tone='subtle' style={{ gap: spacing['4'] }}>
                    ...
                    <Text tone='muted' variant='caption'>
                      {new Date(test.createdAt).toLocaleDateString()} â€¢ status: {test.status ?? 'completed'}
                    </Text>
                    <Text tone='muted' variant='caption'>
                      Recommended: {test.recommendedCount} â€¢ Purchased: {test.purchasedCount}
                    </Text>
                  </Card>
                ))
              )}
