...
const UPLOAD_DIR = path.join(process.cwd(), 'public', 'uploads', 'blocks')
...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > MAX_SIZE_BYTES) {
      return fail('UPLOAD_TOO_LARGE', 'Image must be under 6 MB.', 413)
    }
    ...
    const filename = `block-${Date.now()}-${Math.random().toString(36).slice(2, 7)}.${ext}`
    const filepath = path.join(UPLOAD_DIR, filename)

    await fs.writeFile(filepath, Buffer.from(bytes))

    const url = `/uploads/blocks/${filename}`
    return ok({ url })
  } catch (cause) {
    return fail(
      'UPLOAD_UNEXPECTED',
      ...
      500,
      { scope: 'POST /api/admin/cms/blocks/upload', cause }
    )
  }
}
