...
const bootstrapRelease = read('scripts/bootstrap-release.mjs')
const providerReadiness = read('scripts/verify-provider-readiness.mjs')
...
expectIncludes('package exposes release bootstrap', packageJson, '"bootstrap:release"')
expectIncludes('package exposes provider readiness verifier', packageJson, '"verify:provider-readiness"')
expectIncludes('delivery verifier has devops deployment gate', deliveryVerifier, "'devops-deployment'")
expectIncludes('delivery verifier has provider readiness gate', deliveryVerifier, "'provider-readiness'")
...
expectIncludes('new-client emits direct database url placeholder', newClient, 'DIRECT_URL=')
expectIncludes('new-client checklist includes staging', newClient, 'staging Vercel')
expectIncludes('seed-admin requires env email', seedAdmin, "requiredEnv('ADMIN_EMAIL')")
expectIncludes('seed-admin requires env password', seedAdmin, "requiredEnv('ADMIN_PASSWORD')")
expectIncludes('seed-admin is idempotent', seedAdmin, 'prisma.user.upsert')
expectIncludes('release bootstrap validates Prisma schema', bootstrapRelease, "'Prisma schema validates'")
expectIncludes('release bootstrap requires explicit apply', bootstrapRelease, '--apply')
expectIncludes('release bootstrap supports production target', bootstrapRelease, 'production')
expectIncludes('provider readiness reports customer-ready status', providerReadiness, 'customer-ready')
expectIncludes('provider readiness reports demo-only blockers', providerReadiness, 'demo-only blockers')
...
expectIncludes('staging runbook covers Vercel direct url env var', stagingRunbook, 'vercel env add DIRECT_URL preview')
expectIncludes('staging runbook covers Vercel deploy', stagingRunbook, 'vercel deploy')
expectIncludes('staging runbook covers admin seed', stagingRunbook, 'yarn seed:admin')
