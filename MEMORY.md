...
- DB audit shows Better Auth tables use lowercase names (user, tenant, session, account, verification, tenant_user, app_auth_role_mapping) — these are present, not missing.
...
- RLS policies use `COALESCE(current_setting('app.current_tenant_id', true), 'default')` — reads/writes default to 'default' tenant.
...
- Customer users list fix: removed `where: { role: { not: 'customer' } }` from `GET /api/admin/users` — now returns all users, client filters by role.
