...
## 2026-05-07 Production DB Schema Drift Fix — COMPLETED
...
- DB audit shows 34/41 expected tables present; 7 "missing" tables (User, Tenant, Session, Account, Verification, TenantUser, AppAuthRoleMapping) exist with lowercase names — Better Auth naming convention, not real missing tables.
...
- RLS policies: All 29 policies use `COALESCE(current_setting('app.current_tenant_id', true), 'default')` — reads/writes default to 'default' tenant
- All Prisma models have `tenantId @default("default")` — inserts work without explicit tenantId
- Services use `resolveTenantContext()` which returns 'default' when TENANT_ID env not set
- **No RLS blocking issues** — system works correctly for single-tenant deployment
...
node scripts/verify-functional-storefront.mjs [PASS — all 25 checks including cart page, checkout page, add-to-cart, checkout quote with referral, COD order placement, order history]
```

### Verification (continued)
- Added cart/body items drift warning in `checkout-quote.service.ts` — logs `cart_body_items_mismatch` when scoped cart differs from body items.
...
- Ran `prisma migrate deploy` against production Neon database — all 8 migrations already applied.
...
- GitHub token uses `{env:GITHUB_MCP_PAT}` — no secrets committed.
